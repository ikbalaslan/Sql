...
--9)Select the records whose product name is not Orange or Apple or Palm
...
SELECT*FROM customers_products
WHERE product_id NOT BETWEEN 20 AND 25; --20 and 30 are exclusive
...
UPDATE customers_likes
set customer_name = 'No name'
WHERE EXISTS (SELECT liked_product FROM customers_likes WHERE liked_product in('Orange','Pineapple,Apple'));
...
WHERE EXISTS (SELECT customer_name FROM customers_likes WHERE liked_product = 'Orange');
